...
jsonlint data_bags/private_keys/ops_users.json > /dev/null &&  grep `cat verify_password` data_bags/private_keys/ops_users.json > /dev/null
if [ $? -eq 0 ]; then
echo -e "\e[1;32mops_users file is sucessfully updated\e[0m"
echo "###########################################"
rm -rf back_up/ops_users_new.yaml
rm -rf verify_password
else
...
sed -i '$d' data_bags/private_keys/ops_users.json; sed -i '$d' data_bags/private_keys/ops_users.json
sed -i '$a\ }\n}' data_bags/private_keys/ops_users.json
...
cat verify_password >> $username.private
...
source scripts/templates/usr_mgmt_password_yaml.template "${lines[@]}"
